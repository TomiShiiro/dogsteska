...
class Dogsteska:
    ...
    async def Move(self, x:float, y:float, yaw:float):
        """ 
    ...
    async def GetToDestination(self, dest:Destination):     
        ...
        await self.Move(dest.x, dest.y, dest.yaw)
        
    async def ReturnToStart(self):
        await self.GetToDestination(self.start_position)
...
def start_asyncio(fce):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(fce())
...
def main():    
    ...
    task = threading.Thread(target=start_asyncio, args=(dogsteska.dogsteska_control_thread,), daemon=True)
    task_v = threading.Thread(target=start_asyncio, args=(dogsteska.video_stream_thread,), daemon=True)
    task.start()
    task_v.start()
